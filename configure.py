...
def printLogHeader():
    ...
    printLog(f'Platform: {platform.platform()}');
    printLog('');
...
class ToolCheck(CheckBase):
    ...
    def checkCallback_GSOAPSources(self):
        ...
        if g_oEnv['VBOX_WITH_GSOAP'] == '':
            self.printVerbose(1, 'Skipping, as GSOAP is disabled');
    ...
    def checkCallback_VisualCPP(self):
        ...
        if sVCPPVer:
            ...
            if asMatches:
                ...
                for _, _, sScheme, sDesc in asMatches:
                    ...
                    for sCurArch, curTuple in g_mapWinVSArch2Dir.items():
                        ...
                        # Visual Studio and the (external) Build Tools have different directory structures.
                        # So we have to guess which one it is.
                        sVCPPArchBaseDir = os.path.join(sVCPPPath, sDirHost, sDirArch);
                        sVCPPArchBinDir  = os.path.join(sVCPPArchBaseDir, 'bin'); # Used by Visual Studio installer.
                        if not pathExists(sVCPPArchBinDir):
                            sVCPPArchBinDir = os.path.join(sVCPPPath, 'bin', sDirHost, sDirArch); # Used by internal build tools.

                        if pathExists(sVCPPArchBinDir):
                            g_oEnv.set(f'PATH_TOOL_{sScheme}{sCurArch.upper()}', sVCPPArchBinDir);
                            if g_oEnv['KBUILD_TARGET_ARCH'] == sCurArch:
                                ...
                                g_oEnv.prependPath('PATH', sVCPPArchBinDir);
                                # Same goes for the libs.
                                g_oEnv.prependPath('LIB', os.path.join(sVCPPPath, 'lib', sCurArch));
                        else:
                            self.printError(f"Path '{sVCPPArchBinDir}' not found!");
                            self.printError( "Did you forget to install the matching C++ build tools?");
            ...
            g_oEnv.prependPath('INCLUDE', os.path.join(sVCPPPath, 'include')); # r'C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\VC\Tools\MSVC\14.16.27023\include');
            g_oEnv.prependPath('LIB', os.path.join(sVCPPPath, 'lib', g_mapWinVSArch2Dir.get(g_oEnv['KBUILD_TARGET_ARCH'])[0])); # r'C:\Program Files (x86)\Microsoft Visual Studio\2017\BuildTools\VC\Tools\MSVC\14.16.27023\lib\x64');

            if sVCPPArchBinDir:
                g_oEnv.set('config_c_compiler',   os.path.join(sVCPPArchBinDir, 'cl.exe'));
                g_oEnv.set('config_cpp_compiler', os.path.join(sVCPPArchBinDir, 'cl.exe'));
...
int main()
{
    ...
    return 0;
        ...
        if compileAndExecute('Python Dev', g_oEnv['KBUILD_TARGET'], g_oEnv['KBUILD_TARGET_ARCH'], [ asPathInc ], [ sLibDir ], [ ], [ sLdLib ], sCode):
